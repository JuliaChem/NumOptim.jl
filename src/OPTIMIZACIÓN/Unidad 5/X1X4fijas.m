...
Xo={2.6708,-.9399,1.3904,-.1649,.7488};
XI1=[2.6708,-.1649]';
XD1=[-.9399,1.3904,.7488]';
tam=length(xi);
tem=length(xd);
for i=1:tam  %% 
...
y2=x2 + (l*DELTAD(1,1));  %RESPECTO A X2
...
dh_dxd=inv(subs(dh_dxd,{x1,x2,x3,x4,x5},{XI(1,1),XD(1,1),XD(2,1),XI(2,1),XD(3,1)}));
for i=1:length(h)
Hr(i,:)=vpa(subs(h(i),{x1,x2,x3,x4,x5},{XI(1,1),XD(1,1),XD(2,1),XI(2,1),XD(3,1)}));
end
Xdk= XD - ((dh_dxd))*Hr;
err1= abs((XD-Xdk));
XD(1,1)=Xdk(1,1);
...
Xnueva=[XI(1,1), XD(1,1), XD(2,1), XI(2,1), XD(3,1)]'
Xorign=[XI1(1,1),XD1(1,1),XD1(2,1),XI1(2,1),XD1(3,1)]'
F_Xnueva=vpa(subs(fx,{x1,x2,x3,x4,x5},{XI(1,1), XD(1,1), XD(2,1), XI(2,1), XD(3,1)}))
F_Xorign=vpa(subs(fx,{x1,x2,x3,x4,x5},Xo))
for i=1:length(h)
Hrest(i,:)=vpa(subs(h(i),xx_var,{XI(1,1), XD(1,1), XD(2,1), XI(2,1), XD(3,1)}));
end
Hrest